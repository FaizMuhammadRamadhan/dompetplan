  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-3 shadow-lg">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title fw-semibold" id="modalTambahLabel">Tambah Tanggungan</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form action="{{ route('master-tanggungan.store') }}" method="POST">
        @csrf
        <div class="modal-body p-4">
            <div class="mb-3">
                <label class="form-label">Nama Tanggungan</label>
                <input type="text" name="nama_tanggungan" value="{{ old('nama_tanggungan') }}" class="form-control">
                @error('nama_tanggungan') <small class="text-danger">{{ $message }}</small> @enderror
            </div>

            <div class="mb-3">
                <label class="form-label">Tagihan Perbulan (Rp)</label>
                <input type="number" name="tagihan_perbulan" value="{{ old('tagihan_perbulan') }}" class="form-control" min="0">
                @error('tagihan_perbulan') <small class="text-danger">{{ $message }}</small> @enderror
            </div>

            <div class="mb-3">
                <label class="form-label">Lama Cicilan</label>
                <input type="number" name="lama_cicilan" value="{{ old('lama_cicilan') }}" class="form-control" min="0">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="tanggal_jatuh_tempo_perbulan" value="{{ old('tanggal_jatuh_tempo_perbulan') }}" class="form-control">
